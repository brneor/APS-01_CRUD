...
namespace APS01_Dominio.DAOs
{
    public abstract class BaseDAO<T> : IDAO<T> where T : Model
    {
        ...
        {
            ExecutarComando(GetComandoId(GetSqlDelete(), Convert.ToInt32(obj.Codigo)));
        }
        ...
        {
            var cmd = GetComandoId(sql, Convert.ToInt32(obj.Codigo));
        ...
        private static string GetStringConexao()
        {
            return @"Integrated Security=SSPI;Persist Security Info=False;" +
                   @"Initial Catalog=Aps01;Data Source=localhost\SQLEXPRESS";
        }
    }
}
